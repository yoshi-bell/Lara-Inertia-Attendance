...
*   **Eloquent モデルの移植:**
    ...
    *   旧プロジェクトからリレーション（hasMany, belongsTo）およびビジネスロジック（休憩時間・実働時間の計算アクセサ）を忠実に移植。
    *   **型安全性の強化:** PHP DocBlock によるリレーションの戻り値型定義、および最新の `casts()` メソッドによる日付・時刻の自動変換を設定。
    *   `User` モデルを更新し、`is_admin` への対応と勤怠記録とのリレーションを追加。

### 4. 次のステップ
*   開発用シーダーの作成と実行（データ投入）。
*   Phase 3: 一般ユーザー機能の実装。打刻画面（React/Inertia）の構築。
