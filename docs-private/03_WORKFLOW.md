...
**現在のアクティブタスク:** 管理者用スタッフ一覧の抽象化とバリデーション基盤の全面適用
**ステータス:** 2026-02-23 汎用データテーブル (`AppDataTable`) を開発し、`CorrectionList` および `AttendanceTable` への移行を完遂。複雑なカレンダー形式の表示も抽象化に成功。テスト 139 件合格。次は管理者用スタッフ一覧を移行し、テーブル表示の抽象化を完了させる。
