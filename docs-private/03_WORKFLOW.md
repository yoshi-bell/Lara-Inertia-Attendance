...
**現在のアクティブタスク:** 認証画面 (Login/Register) への Zod 統合の実施
**ステータス:** 2026-02-20 バリデーションメッセージの SSOT 化 (JSON) を完了。全ての FormRequest と Zod スキーマが一一ソースから文言を取得する体制へ移行。テスト 109 件合格。次は認証画面の UI に Zod を適用する。
