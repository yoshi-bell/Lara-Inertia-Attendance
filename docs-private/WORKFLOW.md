...
**現在のアクティブタスク:** 緊急バグ修正（タイムゾーン・時間計算ロジック）
**ステータス:** 申請承認フローの実装および詳細画面のリファクタリングが完了。打刻時間と計算ロジックに重大な不備（時差・マイナス値・0:00固定）を発見。次回、最優先で修正を実施。
...
* [ ] **スタッフ管理 (US012, US013):** スタッフ一覧、個人別月次勤怠、CSVエクスポート (FN045)。

### [緊急] バグ修正タスク (Bug Fixes)
* [ ] **タイムゾーン修正:** サーバー・DB・アプリ設定を日本時間 (Asia/Tokyo) に統一。
* [ ] **休憩時間計算修正:** 休憩時間がマイナスになる等のロジック不備を修正。
* [ ] **合計時間計算修正:** 勤務合計時間が 0:00 になる問題を修正。
