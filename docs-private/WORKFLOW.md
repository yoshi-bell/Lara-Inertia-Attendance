...
**現在のアクティブタスク:** [割り込み] 当日勤怠データの修正制限 (ガード強化)
**ステータス:** E2Eテスト中に「当日データの不完全性」を考慮した仕様変更（閲覧可・編集不可）を決定。UI制御、サーバーガード、既存テスト修正の3工程を実施中。
...
* [ ] **[E2E] シナリオ完遂:** 昨日以前のデータを自動選別する修正・承認フローの完走。
...
* [ ] **[E2E] シナリオ網羅:**
    * [x] 打刻フルサイクルテスト (出勤 → 休憩入 → 休憩戻 → 退勤)
    * [x] 月次一覧・月選択ピッカーの動的な表示更新検証
    * [ ] 一般ユーザーによる修正申請送信と、その後のUI編集制限の検証
    * [ ] 管理者による日次一覧・DatePicker を用いた日付切り替え検証
    * [ ] 管理者による修正申請の承認アクションと、勤怠データへの反映検証
* [ ] **[Unit] Vitest 導入:** 複雑なビジネスロジックや共通コンポーネントの単体テスト。
