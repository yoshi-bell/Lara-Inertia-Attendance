...
**現在のアクティブタスク:** 打刻画面(Attendance/Index)の React 化と Shadcn/ui 導入
**ステータス:** データ層の移植完了(Phase 2完了)。フロントエンド機能の実装を開始。
...
* [ ] **打刻画面:** `Attendance/Index` ページの React 化（Shadcn/ui 使用）。
* [ ] **打刻ロジック:** `AttendanceController` の移植とテスト実装。
* [ ] **勤怠一覧・詳細:** `Attendance/List`, `Detail` の Inertia 化。
* [ ] **修正申請:** 申請フローの実装。

### Phase 4: 管理者機能の実装 (Admin Features)
* [ ] **管理者認証:** マルチガード設定とログイン画面の実装。
* [ ] **管理者ダッシュボード:** 日次勤怠一覧の実装。
* [ ] **承認機能:** 修正申請の承認・却下ロジック。
* [ ] **CSVエクスポート:** 勤怠データのCSV出力機能。

### Phase 5: 仕上げと品質強化 (Refinement)
