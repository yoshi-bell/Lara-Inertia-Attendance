    ...
    return (
        <div className="px-4 md:px-[60px]">
            ...
            <div className={rowStyle}>
                ...
                <div className={contentStyle}>
                    <div className="flex flex-wrap items-center gap-4 md:gap-10">
                        {pendingCorrection ? (
                            <>
                                <span className="w-28 text-center">{formatTimeForInput(pendingCorrection.requested_start_time)}</span>
                                <span className="mx-4 font-normal text-gray-400">〜</span>
                                <span className="w-28 text-center">{formatTimeForInput(pendingCorrection.requested_end_time)}</span>
                            </>
                        ) : (
                            <>
                ...
                <div key={rest.id} className={rowStyle}>
                    ...
                    <div className={contentStyle}>
                        <div className="flex flex-wrap items-center gap-4 md:gap-10">
                            {pendingCorrection ? (
                                <>
                                    <span className="w-28 text-center">{formatTimeForInput(pendingCorrection.rest_corrections.find(rc => rc.rest_id === rest.id)?.requested_start_time)}</span>
                                    <span className="mx-4 font-normal text-gray-400">〜</span>
                                    <span className="w-28 text-center">{formatTimeForInput(pendingCorrection.rest_corrections.find(rc => rc.rest_id === rest.id)?.requested_end_time)}</span>
                                </>
                            ) : (
                                <>
                                    <div className="flex flex-col">
            ...
            <div className={`${rowStyle} items-start border-b-0 py-8`}>
                ...
                <div className={contentStyle}>
                    {pendingCorrection ? (
                        <p className="text-sm font-bold leading-relaxed text-black">{pendingCorrection.reason}</p>
                    ) : (
                        <div>
                            <textarea
                                value={data.reason}
