...
**現在のアクティブタスク:** 汎用データテーブル (AppDataTable) の開発によるリスト表示の抽象化
**ステータス:** 2026-02-23 コンポーネントの UI/ドメイン分離を完遂。ディレクトリ構造を整理し、パスの整合性を機械的に保証。全テスト 139 件合格。次は Phase 4 の核心である「汎用データテーブル」の実装に着手する。
