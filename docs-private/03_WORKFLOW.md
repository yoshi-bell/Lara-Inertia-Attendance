...
**現在のアクティブタスク:** コンポーネントの UI/ドメイン分離とアーキテクチャの洗練
**ステータス:** 2026-02-23 プロジェクト内の全 5 箇所の一覧表示を `AppDataTable` へ移行し、表示基盤の完全な統一を完遂。共通バリデーションユーティリティも全認証画面へ適用済み。テスト 135 件合格。次はコンポーネントの物理的な再配置 (UI/Domain分離) を深化させ、アーキテクチャを洗練させる。
...
* [x] **型チェーンの完成:** Service → Data → Inertia → Props の一貫した型安全性の確保。

### Phase 3: ランタイム安全性の確保 (Zod 統合)
* [x] **スキーマ駆動開発:** `CorrectionForm` 等への Zod 導入と `z.infer` による型生成。
* [x] **バリデーションの統合:** フロントエンド即時検証とバックエンド検証の役割分担。
* [x] **認証全画面の刷新:** 一般・管理者のログイン・登録に加え、パスワードリセット系 (ForgotPassword, ResetPassword, ConfirmPassword) の Zod 化とデザイン統一。

### Phase 4: 抽象化とアーキテクチャの洗練
* [x] **バリデーション共通化:** Zod と Inertia を繋ぐ `performZodValidation` ユーティリティの開発。
* [ ] **ジェネリクス・コンポーネント:** 汎用的なリスト部品やテーブルラッパーの抽出。
* [ ] **Pragmatic Atomic Design:** `Components/ui` (Shadcn) とドメイン特定コンポーネントの分離。

### Phase 5: 最終ブラッシュアップ
* [x] **ファビコン修正:** ブラウザタブのアイコンを Laravel 標準からプロジェクト独自のものへ変更。
* [ ] **コメント規則の徹底適用:** 全ファイルに対して最新のコメント原則 (ゾンビコード禁止、Whyの明記等) を適用。
