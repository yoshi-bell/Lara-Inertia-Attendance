...
### 4. テストコードの高度化と規約遵守 (Refined Testing)
- **リファクタリング:** `Register.test.tsx` の Inertia モックを `React.useState` を用いた「生きたモック」へと刷新。
- **効果:** 手動の `rerender()` を完全に排除し、React のライフサイクルに基づいた自然な状態遷移の検証を実現。
- **規約遵守:** `any` 型を完全に排除し、`unknown` や具体的な型定義へ移行。未使用変数の削除を含め、ESLint (Grade S) の品質基準をテストコードにおいても達成。

---

## 2026-02-20 (続き): 認証画面のリファクタリング拡大とコメント規則の刷新

### 1. ログイン画面 (Login.tsx) への Zod 統合
- **実装内容:** 
    - `Login.tsx` に `loginSchema` を適用。送信前のクライアントサイド・バリデーションを実現。
    - `Register.tsx` で確立した「最初のエラーメッセージを優先する」ロジックを導入し、UX を統一。
- **単体テスト:** 
    - `Login.test.tsx` を新規作成。`useState` を用いた「生きたモック」パターンを初手から採用し、高品質な検証体制を構築。

### 2. コーディング規約 (コメント規則) の高度化
- **規約刷新:** `04_CODING_RULES.md` を更新し、以下の 4 原則を追加。
    1. ゾンビコード（コメントアウトされたコード）の禁止。
    2. 外部ソース（USxxx, GitHub Issue）へのリンクによるトレーサビリティ確保。
    3. 妥協（ワークアラウンド）の理由と解除条件の明記。
    4. AI エージェントへの文脈提供（Prompts as Documentation）。
- **実適用:** `Auth` ディレクトリ内の主要ファイル（Login, Register, VerifyEmail）に対して、新しい規則に基づいた意図の記述とクリーンアップを実施。

### 3. 認証全画面リファクタリングの方針決定
- **ロードマップ拡張:** `ForgotPassword`, `ResetPassword`, `ConfirmPassword` についても Phase 3 の対象に含め、デザイン刷新と Zod 統合を行うことを決定。
- **UI/UX 向上:** ブラウザタブのファビコン修正を将来のタスクとしてワークフローに明文化。

### 4. 品質保証の継続
- **全テスト合格:** 構造変更と規約適用後も、全 117 件のテスト (Feature 63, Unit 45, E2E 9) が 100% 合格することを確認。

---

## 2026-02-20 (続き): 認証全画面のリファクタリング完遂とテストの高度化

### 1. 管理者ログインおよびパスワードリセット系画面の刷新
- **実装対象:** `Admin/Login.tsx`, `Auth/ForgotPassword.tsx`, `Auth/ResetPassword.tsx`, `Auth/ConfirmPassword.tsx`
- **内容:** 
    - 全画面を「Atte」ブランドの共通デザイン（黒ボタン、太字、大きなフォント）に統一。
    - 各画面に Zod スキーマを統合し、送信前の即時バリデーションを実装。
    - `Login.tsx` (一般) に `ForgotPassword` へのリンクを設置し、ユーザー導線を改善。
- **効果:** アプリケーション内の全認証フローにおいて、一貫したブランド体験と「Grade S」のランタイム安全性を実現。

### 2. 「生きたモック (useState 版)」によるテストの高度化
- **リファクタリング:** 全ての新規・既存のページ単体テストにおいて、Inertia の `useForm` モックを `React.useState` を使用した形式へ刷新。
- **成果:** 手動の `rerender()` を完全に排除。React 本来のライフサイクル（入力 -> 状態変化 -> 再描画）をテスト内で再現し、より現実に即した高品質な検証体制を確立。

### 3. スキーマ駆動開発の深化
- **`authSchema.ts` の拡張:** リセットトークン検証やパスワード一致確認を含む、全認証パターンを網羅するスキーマを構築。
- **DRY 原則の徹底:** メッセージ優先順位ロジック（最初のエラーを表示）を全画面で共通適用。

### 4. 最終品質確認
- **全テスト合格:** 認証全域に及ぶ大規模なリファクタリングの後、計 131 件のテスト (Feature 63, Unit 59, E2E 9) が 100% 合格することを確認。

---

## 2026-02-20 (続き): Phase 4 抽象化の着手とバリデーション基盤の洗練

### 1. バリデーション・ロジックの共通化 (Logic Abstraction)
- **ユーティリティ実装:** `resources/js/lib/validation.ts` を新設。
- **内容:** Zod の `safeParse` 結果を Inertia の `setError` 形式に変換する共通関数 `performZodValidation` を開発。
- **柔軟な設計:** `joinMessages` オプションを導入し、ログイン画面等での「最初のエラー優先」と、詳細画面等での「全エラー結合表示」を切り替え可能にした。これにより、複雑な不備を的確にユーザーへ伝える UX を実現。

### 2. 勤怠修正バリデーションの完全性向上
- **ロジック移植:** バックエンドにのみ存在していた「休憩時間同士の重複チェック」を `correctionSchema.ts` (Zod) へ完全移植。
- **責任の分離とテスト:** 
    - `correctionSchema.test.ts` を新設し、バリデーションロジック単体での健全性を検証。
    - 休憩の重複、範囲外、入力漏れ等のエッジケースを網羅し、メンテナンス性を向上させた。
- **実適用:** `useCorrectionForm.ts` をリファクタリングし、新設したユーティリティを介した高度な検証フローへ移行。

### 3. ビジュアル・アイデンティティの確立
- **ファビコン更新:** `public/favicon.ico` をプロジェクト独自のものへ差し替え。
- **HTML 同期:** `app.blade.php` を修正し、ブラウザタブに Atte オリジナルのアイコンが表示されるように設定。

### 4. 品質保証の継続
- **全テスト合格:** 基盤ロジックの共通化とスキーマ強化の後、計 137 件のテスト (Feature 63, Unit 65, E2E 9) が 100% 合格することを確認。

### 5. 環境依存の解消とビルドの安定化
- **Zod v4 対応:** CI 環境において Zod v4 の型定義変更（SafeParseReturnType の非推奨化など）に起因するビルドエラーを特定。
- **修正:** `performZodValidation` において `z.ZodSafeParseSuccess` および `z.ZodSafeParseError` を使用する形式へリファクタリング。これにより、ローカルと CI の両環境で型安全なビルドを保証。
- **最終検証:** 全単体テストおよびプロダクションビルドのパスを確認。計 139 件のテストが 100% 合格する「完全なグリーン」を確立。

---

## 2026-02-23: Phase 4 抽象化の進展とディレクトリ構造の洗練

### 1. コンポーネントの UI/ドメイン分離 (Pragmatic Atomic Design)
- **再配置:** `resources/js/Components` 直下のファイルを整理し、業務知識を持つコンポーネントを `domain/` サブディレクトリへ移動。
- **分類:** 
    - `domain/`: `AttendanceTable`, `CorrectionForm`, `MonthPicker`, `Header`, `Navigation` 等。
    - `ui/`: (既存) `Button`, `Input`, `Calendar` 等の汎用部品。
- **目的:** 責務の明確化による保守性の向上と、将来のジェネリクス化（抽象化）に向けた基盤整理。

### 2. インポートパスの一斉修正と整合性確保
- **自動修正:** `sed` コマンドおよび手動修正により、Pages, Layouts, Tests 全域のインポートパスを新しい構造へ同期。
- **型安全性の検証:** `npx tsc --noEmit` およびプロダクションビルドを実行し、パス不整合によるエラーが 0 件であることを機械的に保証。

### 3. テスト層による完全なデグレード防止
- **単体テスト:** `Vitest` 63 件の全合格を確認。特に `domain/` へ移動したテストファイルが正常に動作することを証明。
- **E2E テスト:** `Playwright` 9 シナリオの全合格を確認。実際のブラウザ上での全認証・打刻・承認フローの正常動作を最終証明。

### 4. 最終品質ステータス
- **合格数:** 計 139 件のテスト（Feature 63, Unit 67, E2E 9）が 100% 合格。構造変更後のデグレード 0 を達成。

---

## 2026-02-23 (続き): 汎用データテーブルの開発と抽象化の深化 (Phase 4)

### 1. 汎用データテーブル (AppDataTable) の開発
- **実装:** `resources/js/Components/ui/AppDataTable.tsx` を新設。
- **特徴:** 
    - Generics (`<T>`) を活用し、あらゆるデータ型に対して一貫したテーブル表示を提供。
    - 列定義（columns）による宣言的な設計を採用し、UI 枠組みと表示ロジックを分離。
- **型定義の洗練 (Clear Code):** 
    - 難解な型ハック `(string & {})` を `LooseStringAutocomplete<T>` という型エイリアスに隠蔽。
    - 「中間式の明示」により、高度な型定義自体の可読性と意図（Why）を明確化した。

### 2. 既存一覧画面の移行 (CorrectionList)
- **リファクタリング:** `CorrectionList.tsx` を `AppDataTable` ベースに刷新。
- **効果:** JSX 内のループやテーブルタグの重複を排除。列定義を説明変数として抽出することで、コード自体が仕様解説となる「明快なコード」へと昇華。

### 3. コーディング規約のアップデート
- **規約刷新:** `04_CODING_RULES.md` に「明快なコード (Clear Code) の優先」セクションを追加。
- **内容:** 説明変数の導入、型エイリアスによる意図明示、AI エージェント向けの文脈提供を義務付け、プロジェクトの保守性を一段階引き上げた。

### 4. 動作確認と品質保証
- **E2E テスト:** 構造変更後も Playwright 9 シナリオが 100% 合格することを確認。DOM 構造の変化による影響がないことを実証。
- **型チェック:** `npx tsc --noEmit` により、Generics を用いた高度な抽象化が型安全に実装されていることを確認。

### 5. カレンダー形式テーブルの抽象化 (AttendanceTable)
- **リファクタリング:** 最も複雑な表示ロジックを持つ `AttendanceTable.tsx` を `AppDataTable` 基盤へ完全移行。
- **抽象化の深化:** 勤怠データが Nullable なカレンダー形式の配列に対し、`render` プロパティを活用した宣言的な条件分岐（詳細リンクの有無等）を実装。
- **デザインの継承:** `tracking-[3px]` 等の旧プロジェクト固有のスタイルを、汎用コンポーネントの `tableClassName` を介して完璧に引き継いだ。
- **品質の証明:** 内部構造の全面刷新後も、既存の単体テスト (`AttendanceTable.test.tsx`) および E2E テストが 100% 合格。振る舞いの不変性を保証した。

### 6. スタッフ一覧の抽象化と全テーブル表示の基盤統一完了
- **リファクタリング:** `Admin/Staff/List.tsx` を `AppDataTable` へ移行。
- **統一感の達成:** これにより、プロジェクト内の全ての一覧画面（一般勤怠、管理者日次、スタッフ別月次、スタッフ一覧、申請一覧）が単一の汎用テーブル基盤の上に統合された。
- **保守性の向上:** テーブルのデザインや共通挙動の変更が `AppDataTable.tsx` 一箇所の修正で全画面に波及する、極めて DRY な構造を確立。
- **最終品質ステータス:** 計 135 件のテスト（Feature 63, Unit 63, E2E 9）が 100% 合格。

### 7. バリデーションユーティリティの全面適用と品質の最終化
- **適用拡大:** `Login`, `Register`, `Admin/Login`, `ForgotPassword`, `ResetPassword`, `ConfirmPassword` の全認証画面に `performZodValidation` を適用。
- **効果:** 各コンポーネント内の冗長なバリデーションループを排除。アプリ全体で一貫したエラー表示挙動（メッセージ優先順位等）を保証。
- **品質証明:** 大規模な共通化リファクタリングの後、全レイヤーのテスト (計 135 件) が 100% 合格することを実機環境で証明。
- **言語の厳密化:** `utils.d.ts` において「目的（エンジニアとしての成長）」と「目標（Findyスコア）」の定義を修正。言葉の解像度を高めることがコード品質（可読性）に直結することを再認識。
