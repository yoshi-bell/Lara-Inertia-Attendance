...
**現在のアクティブタスク:** ログイン・登録画面への共通バリデーションユーティリティの適用
**ステータス:** 2026-02-23 汎用データテーブル (`AppDataTable`) を用い、プロジェクト内の全 5 箇所の一覧表示を抽象化。テーブルタグの直接記述を排除し、宣言的な設計への移行を完遂。テスト 135 件合格。次は認証画面のバリデーションロジックを共通ユーティリティへ移行する。
